...
async function init() {
  clearLegacyStorage();
  artworks = cloudClient ? await loadCloudArtworks() : await loadArtworks();
  render();
}
...
artworkForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const artwork = cloudClient
      ? await createCloudArtwork(imageBlob, formData)
      : await createLocalArtwork(imageBlob, formData);
...
dialogLike.addEventListener("click", async () => {
  if (!activeArtworkId) {
    return;
  }
  await toggleLike(activeArtworkId);
...
commentForm.addEventListener("submit", async (event) => {
  ...
  if (cloudClient && artwork.remote) {
    const updatedComments = await addCloudComment(artwork.id, text);
    if (updatedComments) {
      artwork.comments = updatedComments;
    }
  } else {
...
async function toggleLike(id) {
  ...
  const previousLiked = artwork.liked;
  const previousLikes = artwork.likes;
  const delta = artwork.liked ? -1 : 1;
  ...
  if (cloudClient && artwork.remote) {
    try {
      const { data, error } = await cloudClient.rpc("increment_artwork_likes", {
      ...
      const savedLikes = Number(data);
      if (Number.isFinite(savedLikes)) {
        artwork.likes = savedLikes;
      }
    } catch (error) {
      artwork.liked = previousLiked;
      artwork.likes = previousLikes;
      alert("The like could not be saved. Please try again.");
      return;
    }
  }
...
async function createLocalArtwork(imageBlob, formData) {
  const id = createId();
...
async function createCloudArtwork(imageBlob, formData) {
  const imageKey = `${createId()}.jpg`;
  const title = cleanText(formData.get("art-title")) || "Untitled Artwork";
  const artist = cleanText(formData.get("artist-name")) || "Anonymous Artist";
  const mood = formData.get("art-mood") || "abstract";
  const story = cleanText(formData.get("art-story")) || "The artist has left the outside story open for now.";
  const { error: uploadError } = await cloudClient.storage.from(CLOUD_BUCKET).upload(imageKey, imageBlob, {
  ...
  const { data: publicImage } = cloudClient.storage.from(CLOUD_BUCKET).getPublicUrl(imageKey);
...
async function loadCloudArtworks() {
  try {
    const { data, error } = await cloudClient
      .from("artworks")
      .select("id,title,artist,mood,story,image_url,likes,comments,created_at")
    ...
    return [...data.map(mapCloudArtwork), ...seedArtworks];
  } catch (error) {
    return await loadArtworks();
  }
}

function mapCloudArtwork(row) {
  return {
    id: row.id,
    title: row.title,
    artist: row.artist,
    mood: row.mood,
    image: row.image_url,
    story: row.story || "The artist has left the outside story open for now.",
...
async function addCloudComment(artworkId, text) {
  const { data, error } = await cloudClient.rpc("add_artwork_comment", {
    artwork_id: artworkId,
    body: text,
  });

  if (error) {
    alert("The comment could not be saved. Please try again.");
    return null;
  }
...
function createCloudClient() {
  const config = globalThis.OIL_SALON_SUPABASE || {};
